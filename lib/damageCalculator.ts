...
export function calculateCombatStats(input: DamageCalculationInput): CombatStats {
  ...
  if (weapon.skill.enabled) {
    weapon.skill.effects.forEach((effect: Weapon['skill']['effects'][number]) => {
...
export function calculateDamage(input: DamageCalculationInput): DamageCalculationResult {
  ...
  if (input.extraBonuses) {
    input.extraBonuses.forEach(bonus => {
      // 生效范围过滤：byCategory 按技能大类，byName 按技能名称
      const filterMode = bonus.filterMode ?? "all";
      if (filterMode === "byCategory") {
        if (!bonus.filterValues || !bonus.filterValues.includes(selectedSkill.skillCategory)) return;
      } else if (filterMode === "byName") {
        if (!bonus.filterValues || !bonus.filterValues.includes(selectedSkill.name)) return;
      }
      switch (bonus.zone) {
        case "倍率提升":
