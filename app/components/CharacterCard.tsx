...
import { getCharacterImagePath } from "@/lib/imageHelper";
...
export const CharacterCard = ({ 
  ...
  if (variant === "display") {
    ...
    return (
      <div className={`w-40 h-50 bg-white rounded-lg flex items-center justify-center overflow-hidden border-4 ${getRarityBorderColor(rarity)}`}>
        <Image
          src={getCharacterImagePath(imageKey)}
          alt={name}
          width={150}
          height={150}
  ...
  return (
    ...
    >
      <div className={`w-46 h-63 rounded mb-1 flex justify-center overflow-hidden`}>
        <Image
          src={getCharacterImagePath(imageKey)}
          alt={name}
          width={300}
      ...
      <p className="text-center text-gray-600">{rarity}星 · {elementType}</p>
    </button>
  );
};
